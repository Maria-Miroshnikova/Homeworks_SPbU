...
bool binarySearch(int indexLeft, int indexRight, int randomArray[], int number)
{
	const int length = indexRight - indexLeft + 1;
	if (length == 1)
	{
		return (number == randomArray[indexLeft]);
	}
	const int indexMiddleElement = indexLeft + length / 2 - 1;
...
void inputRandomArray(int length, int block[], int maximum)
{
	...
	{
		block[i] = rand() * rand() * rand() % maximum;
	}
}
...
{
	quickSort(testArray, 0, testLength - 1);
