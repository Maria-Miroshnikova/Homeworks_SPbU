...
void addListEntry(List* list, WeightedEdge* newElement)
{
	...
	if (entry->edge->distance > newElement->distance)   // если место элемента сразу в голове списка
	{
		list->head = newEntry;
		newEntry->next = entry;
		return;
	...
	while (entry->edge->distance <= newElement->distance)
	{
		...
		else if (entry->next->edge->distance > newElement->distance)   //   если место элемента между рассматриваемым и следующим элементами
		{
...
int findMinNotConqueredNode(List* list, GraphNode** findNode)
{
	...
	while (entry != nullptr)
	{
		if (!entry->edge->endNode->conquered)
		{
			*findNode = entry->edge->endNode;
			return entry->edge->distance;
		}
		entry = entry->next;
	}
