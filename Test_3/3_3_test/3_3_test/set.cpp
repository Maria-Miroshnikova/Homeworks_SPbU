...
void dfsFindMaxSetElement(SetElement* element, int& maxCount, int& maxElement)
{
	if (element == nullptr)
	{
		return;
	}
	dfsFindMaxSetElement(element->leftChild, maxCount, maxElement);
	...
	dfsFindMaxSetElement(element->rightChild, maxCount, maxElement);
}
...
int findMaxSetElement(Set* set)
{
	...
	dfsFindMaxSetElement(set->head, maxCount, maxElement);
