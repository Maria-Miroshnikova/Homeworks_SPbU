...
int addEntry(PhoneBookEntry phoneBook[], PhoneBookEntry newEntry, int *indexLastEntry)
{
	...
	else if (searchEntry(phoneBook, newEntry.phone, false, *indexLastEntry) != -1)
	{
		return 2;
	}
	else if (*indexLastEntry == 100)
	{
		return 3;
	}
	else
...
int findPhone(PhoneBookEntry phoneBook[], char name[], int indexLastEntry)
{
	return searchEntry(phoneBook, name, true, indexLastEntry);
}

int findName(PhoneBookEntry phoneBook[], char phone[], int indexLastEntry)
{
	return searchEntry(phoneBook, phone, false, indexLastEntry);
}
...
void savePhoneBook(PhoneBookEntry phoneBook[], int indexLastEntry)
{
	...
	{
		saveEntry(phoneBook[i], phonebookFile);
	}
