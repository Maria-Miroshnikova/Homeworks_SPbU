...
void tests()
{
	...
	while (!testCommandFile.eof())
	{
		...
		if (command == "1")
		{
			...
			while ((data != "\0") && (data != "\n"))
			{
				assert(addSetElement(testSet, strtol(data.c_str(), nullptr, 10)) == strtol(answer.c_str(), nullptr, 10));
		...
		else if (command == "2")
		{
			...
			while ((data != "\0") && (data != "\n"))
			{
				assert(removeSetElement(testSet, strtol(data.c_str(), nullptr, 10)) == strtol(answer.c_str(), nullptr, 10));
		...
		else if (command == "3")
		{
			...
			while ((data != "\0") && (data != "\n"))
			{
				assert((findSetEelement(testSet, strtol(data.c_str(), nullptr, 10)) != nullptr) == strtol(answer.c_str(), nullptr, 10));
		...
		{
			readDataFromFile(answer, testAnswerFile);
			assert(outputSetElements(testSet, 1, sequence) == strtol(answer.c_str(), nullptr, 10));
		...
		{
			readDataFromFile(answer, testAnswerFile);
			assert(outputSetElements(testSet, 0, sequence) == strtol(answer.c_str(), nullptr, 10));
